...
object MetricMismatch {
  ...
  private final case class NotFoundImpl(expectation: MetricExpectation, availableMetricNames: List[String])
      extends NotFound {
  ...
  private final case class ClosestMismatchImpl(
    ...
    def message: String = {
      ...
      s"${prefix}closest metric '${metric.name}' mismatched:$rendered"
    }
  }
...
object MetricExpectations {
  ...
  def exists(metrics: List[MetricData], expectation: MetricExpectation): Boolean =
  ...
  def find(metrics: List[MetricData], expectation: MetricExpectation): Option[MetricData] =
  ...
  def check(metrics: List[MetricData], expectation: MetricExpectation): Option[MetricMismatch] =
    FlatExpectationMatching.check(metrics, expectation)(_.matches(_), bestMismatch)
  ...
  def checkAll(
    ...
    FlatExpectationMatching.checkAll(metrics, expectations)(_.matches(_), bestMismatch)
  ...
  def missing(metrics: List[MetricData], expectations: List[MetricExpectation]): List[MetricMismatch] =
    FlatExpectationMatching.missing(metrics, expectations)(_.matches(_), bestMismatch)

  /** Returns mismatches for all expectations that could not be matched to distinct collected metrics. */
  def missingDistinct(metrics: List[MetricData], expectations: List[MetricExpectation]): List[MetricMismatch] = {
  ...
  def allMatch(metrics: List[MetricData], expectations: List[MetricExpectation]): Boolean =
    FlatExpectationMatching.allMatch(metrics, expectations)(_.matches(_), bestMismatch)

  /** Returns `true` if every expectation matched a different collected metric. */
  def allMatchDistinct(metrics: List[MetricData], expectations: List[MetricExpectation]): Boolean =
  ...
  def format(mismatches: NonEmptyList[MetricMismatch]): String =
    FlatExpectationMatching.format("Metric expectations", mismatches)(_.message)

  private def bestMismatch(metrics: List[MetricData], expectation: MetricExpectation): MetricMismatch = {
    val candidates = expectation.expectedName match {
      case Some(name) => metrics.filter(_.name == name)
      case None       => metrics
    }

    candidates
      .flatMap(metric => expectation.check(metric).left.toOption.map(metric -> _))
      ...
      .headOption
      .map { case (metric, mismatches) => MetricMismatch.closestMismatch(expectation, metric, mismatches) }
      .getOrElse(MetricMismatch.notFound(expectation, metrics.map(_.name)))
  }
}
