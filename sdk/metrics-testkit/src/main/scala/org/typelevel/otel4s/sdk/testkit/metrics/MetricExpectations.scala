...
object MetricExpectations {
  ...
  def check(metrics: List[MetricData], expectation: MetricExpectation): Option[MetricMismatch] =
    if (exists(metrics, expectation)) None else Some(bestMismatch(metrics, expectation))
  ...
  def missing(metrics: List[MetricData], expectations: List[MetricExpectation]): List[MetricMismatch] =
    expectations.flatMap(expectation => check(metrics, expectation))
          ...
          candidates(index) match {
            ...
            case matches =>
              MetricMismatch.distinctMatchUnavailable(
                ...
                matches.map(indexedMetrics(_).name).distinct
              )
          }
      }.toList
  }
        ...
        case (None, metricIndex) =>
          matching.get(metricIndex) match {
            ...
            case Some(otherExpectationIndex) =>
              augment(otherExpectationIndex, seen + metricIndex, matching).map(_.updated(metricIndex, expectationIndex))
          }
      }
