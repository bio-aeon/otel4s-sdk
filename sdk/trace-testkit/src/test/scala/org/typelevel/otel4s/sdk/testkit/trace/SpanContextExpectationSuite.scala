...
class SpanContextExpectationSuite extends FunSuite {
  ...
  test("exact matches the full span context") {
    val actual = context(sampled = true, remote = true, traceState = TraceState.empty.updated("k", "v"))

    assertEquals(SpanContextExpectation.exact(actual).check(actual), Right(()))
    assertEquals(
      SpanContextExpectation.exact(actual).check(context()),
      Left(
        NonEmptyList.of(
          SpanContextExpectation.Mismatch.SampledMismatch(true, false),
          SpanContextExpectation.Mismatch.RemoteMismatch(true, false),
          SpanContextExpectation.Mismatch.TraceFlagsMismatch(actual.traceFlags, TraceFlags.Default),
          SpanContextExpectation.Mismatch.TraceStateMismatch(actual.traceState, TraceState.empty)
        )
      )
    )
  }
