...
object SpanMismatch {
  ...
  private final case class ClosestMismatchImpl(
    ...
    def message: String = {
      ...
      s"${clue.fold("")(c => s"[$c] ")}closest span '${span.name}' mismatched:$rendered"
    }
  }
...
object SpanExpectations {
  ...
  private def bestMismatch(spans: List[SpanData], expectation: SpanExpectation): SpanMismatch = {
    val candidates = expectation.expectedName match {
      case Some(name) => spans.filter(_.name == name)
      case None       => spans
    }
    ...
    candidates
      ...
      .getOrElse(SpanMismatch.notFound(expectation, spans.map(_.name)))
  }
}
