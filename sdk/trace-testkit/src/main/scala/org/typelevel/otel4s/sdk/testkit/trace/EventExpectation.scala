...
/** A partial expectation for a single [[org.typelevel.otel4s.sdk.trace.data.EventData]].
  *
  * `EventExpectation` is intended for tests where asserting against the full event payload would be too verbose.
...
object EventExpectation {
  ...
  object Mismatch {
    ...
    private[testkit] final case class PredicateMismatch(clue: Option[String]) extends Mismatch {
      def message: String = s"event predicate returned false${clue.fold("")(v => s": $v")}"
    }
  }
  ...
  private final case class Impl(
      ...
      predicates: List[(EventData => Boolean, Option[String])] = Nil
  ) extends EventExpectation {
    def name(name: String): EventExpectation = copy(expectedName = Some(name))
    def timestamp(timestamp: FiniteDuration): EventExpectation = copy(expectedTimestamp = Some(timestamp))
    def attributes(expectation: AttributesExpectation): EventExpectation = copy(expectedAttributes = Some(expectation))
    def attributesExact(attributes: Attributes): EventExpectation =
      copy(expectedAttributes = Some(AttributesExpectation.exact(attributes)))
    def attributesExact(attributes: Attribute[_]*): EventExpectation = attributesExact(Attributes(attributes: _*))
    def attributesSubset(attributes: Attributes): EventExpectation =
      copy(expectedAttributes = Some(AttributesExpectation.subset(attributes)))
    def attributesSubset(attributes: Attribute[_]*): EventExpectation = attributesSubset(Attributes(attributes: _*))
    def attributesEmpty: EventExpectation = attributesExact(Attributes.empty)
    def clue(text: String): EventExpectation = copy(clue = Some(text))
    def where(f: EventData => Boolean): EventExpectation = copy(predicates = predicates :+ (f -> None))
    def where(clue: String)(f: EventData => Boolean): EventExpectation =
      copy(predicates = predicates :+ (f -> Some(clue)))

    def check(event: EventData): Either[NonEmptyList[Mismatch], Unit] =
      ExpectationChecks.combine(
        expectedName.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == event.name) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.NameMismatch(expected, event.name))
        },
        expectedTimestamp.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == event.timestamp) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.TimestampMismatch(expected, event.timestamp))
        },
        expectedAttributes.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(event.attributes.elements))(Mismatch.AttributesMismatch(_))
        },
