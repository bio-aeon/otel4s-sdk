...
/** A partial expectation for a single SDK [[org.typelevel.otel4s.sdk.trace.data.SpanData]].
  *
  * `SpanExpectation` is intended for tests where asserting against the full `SpanData` shape would be too verbose.
  * Unspecified properties are ignored. Event and link matching is expressed through collection-level
  * [[EventSetExpectation]] and [[LinkSetExpectation]] values, which allows multiple collection constraints to
...
object SpanExpectation {
  ...
  object Mismatch {
    ...
    private[testkit] final case class KindMismatch(expected: SpanKind, actual: SpanKind) extends Mismatch {
      def message: String = s"kind mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class StatusMismatch(mismatches: NonEmptyList[StatusExpectation.Mismatch])
        extends Mismatch {
    ...
    private[testkit] final case class ParentSpanContextMismatch(
    ...
    private[testkit] case object MissingParentSpanContext extends Mismatch {
      def message: String = "parent span context mismatch: expected a parent, got <missing>"
    }

    private[testkit] final case class UnexpectedParentSpanContext(actual: SpanContext) extends Mismatch {
      def message: String = s"parent span context mismatch: expected <missing>, got '$actual'"
    }

    private[testkit] final case class StartTimestampMismatch(expected: FiniteDuration, actual: FiniteDuration)
        extends Mismatch {
      def message: String = s"start timestamp mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class EndTimestampMismatch(
    ...
    private[testkit] final case class EndedMismatch(expected: Boolean, actual: Boolean) extends Mismatch {
    ...
    private[testkit] final case class EventsMismatch(
    ...
    private[testkit] final case class LinksMismatch(
    ...
    def check(span: SpanData): Either[NonEmptyList[Mismatch], Unit] =
      ExpectationChecks.combine(
        name.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == span.name) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.NameMismatch(expected, span.name))
        },
        kind.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == span.kind) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.KindMismatch(expected, span.kind))
        },
        status.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(span.status))(Mismatch.StatusMismatch(_))
        },
        spanContext.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(span.spanContext))(Mismatch.SpanContextMismatch(_))
        },
        parentSpanContext.fold(ExpectationChecks.success[Mismatch]) {
          case None =>
            span.parentSpanContext match {
        ...
        startTimestamp.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == span.startTimestamp) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.StartTimestampMismatch(expected, span.startTimestamp))
        },
        endTimestamp.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == span.endTimestamp) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.EndTimestampMismatch(expected, span.endTimestamp))
        },
        ended.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == span.hasEnded) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.EndedMismatch(expected, span.hasEnded))
        },
        attributes.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(span.attributes.elements))(Mismatch.AttributesMismatch(_))
        },
        ExpectationChecks.combine(eventExpectations.map { expectation =>
          ExpectationChecks.nested(expectation.check(span.events.elements.toList))(mismatches =>
            Mismatch.EventsMismatch(mismatches, expectation.clue)
          )
        }),
        ExpectationChecks.combine(linkExpectations.map { expectation =>
          ExpectationChecks.nested(expectation.check(span.links.elements.toList))(mismatches =>
            Mismatch.LinksMismatch(mismatches, expectation.clue)
          )
        }),
        scope.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(span.instrumentationScope))(Mismatch.ScopeMismatch(_))
        },
        resource.fold(ExpectationChecks.success[Mismatch]) { expected =>
          ExpectationChecks.nested(expected.check(span.resource))(Mismatch.ResourceMismatch(_))
        },
