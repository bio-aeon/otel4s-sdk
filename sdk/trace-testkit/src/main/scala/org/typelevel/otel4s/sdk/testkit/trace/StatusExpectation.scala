...
/** A partial expectation for [[org.typelevel.otel4s.sdk.trace.data.StatusData]]. */
sealed trait StatusExpectation {
  ...
  def check(status: StatusData): Either[NonEmptyList[StatusExpectation.Mismatch], Unit]

  /** Returns `true` if this expectation matches the given status. */
  final def matches(status: StatusData): Boolean =
...
object StatusExpectation {
  ...
  object Mismatch {
    ...
    private[testkit] final case class CodeMismatch(expected: StatusCode, actual: StatusCode) extends Mismatch {
      def message: String = s"status code mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class DescriptionMismatch(expected: Option[String], actual: Option[String])
        extends Mismatch {
      def message: String = {
  ...
  private final case class Impl(
    ...
    def check(status: StatusData): Either[NonEmptyList[Mismatch], Unit] =
      ExpectationChecks.combine(
        expectedCode.fold(ExpectationChecks.success[Mismatch]) { expected =>
          if (expected == status.status) ExpectationChecks.success
          else ExpectationChecks.mismatch(Mismatch.CodeMismatch(expected, status.status))
        },
        ExpectationChecks.compareOption(expectedDescription, status.description)(Mismatch.DescriptionMismatch(_, _))
      )
  }
}
