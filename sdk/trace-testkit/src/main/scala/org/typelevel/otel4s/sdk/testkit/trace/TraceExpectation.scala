...
sealed trait TraceExpectation {
  ...
  def childrenMatchMode: TraceExpectation.MatchMode
...
object TraceExpectation {
  ...
  object Mismatch {
    ...
    private[testkit] final case class SpanMismatch(mismatches: NonEmptyList[SpanExpectation.Mismatch])
        extends Mismatch {
      def message: String = s"span mismatch: ${mismatches.toList.map(_.message).mkString(", ")}"
    }

    private[testkit] final case class ChildCountMismatch(expected: Int, actual: Int, actualChildNames: List[String])
        extends Mismatch {
      def message: String =
        s"child count mismatch: expected '$expected', got '$actual'; actual children: [${actualChildNames.mkString(", ")}]"
    }

    private[testkit] final case class MissingChild(expectation: TraceExpectation, availableChildNames: List[String])
        extends Mismatch {
      def message: String =
        s"missing child matching expectation; available children: [${availableChildNames.mkString(", ")}]"
    }

    private[testkit] final case class DistinctChildMatchUnavailable(
    ...
    private[testkit] final case class ChildMismatch(
        actual: SpanData,
        mismatches: NonEmptyList[TraceExpectation.Mismatch]
    ) extends Mismatch {
      def message: String =
        s"trace subtree mismatch for '${actual.name}': ${mismatches.toList.map(_.message).mkString(", ")}"
    }
  ...
  def leaf(current: SpanExpectation): TraceExpectation =
    Impl(current, Nil, MatchMode.Unordered, None)
  ...
  def ordered(current: SpanExpectation, children: TraceExpectation*): TraceExpectation =
    Impl(current, children.toList, MatchMode.Ordered, None)
  ...
  def unordered(current: SpanExpectation, children: TraceExpectation*): TraceExpectation =
    Impl(current, children.toList, MatchMode.Unordered, None)

  private final case class Impl(
      current: SpanExpectation,
      children: List[TraceExpectation],
      childrenMatchMode: MatchMode,
      clue: Option[String]
      ...
      val result = {
        ...
        val childCheck =
          if (children.length != tree.children.length)
            Left(
              NonEmptyList.one(
                Mismatch.ChildCountMismatch(children.length, tree.children.length, tree.children.map(_.span.name))
              )
            )
          else
            childrenMatchMode match {
                          ...
                          candidates(index) match {
                            ...
                            case matches =>
                              Mismatch.DistinctChildMatchUnavailable(
                                children(index),
                                matches.map(tree.children(_).span.name).distinct
                              )
                          }
                      }.toList
                    )
