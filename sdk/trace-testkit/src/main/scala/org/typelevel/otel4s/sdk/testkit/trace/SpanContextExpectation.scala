...
object SpanContextExpectation {
  ...
  object Mismatch {
    ...
    private[testkit] final case class TraceIdMismatch(expected: String, actual: String) extends Mismatch {
      def message: String = s"trace ID mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class SpanIdMismatch(expected: String, actual: String) extends Mismatch {
      def message: String = s"span ID mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class SampledMismatch(expected: Boolean, actual: Boolean) extends Mismatch {
      def message: String = s"sampled mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class RemoteMismatch(expected: Boolean, actual: Boolean) extends Mismatch {
      def message: String = s"remote mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class TraceFlagsMismatch(expected: TraceFlags, actual: TraceFlags) extends Mismatch {
      def message: String = s"trace flags mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class TraceStateMismatch(expected: TraceState, actual: TraceState) extends Mismatch {
      def message: String = s"trace state mismatch: expected '$expected', got '$actual'"
    }

    private[testkit] final case class PredicateMismatch(clue: Option[String]) extends Mismatch {
      def message: String = s"span context predicate returned false${clue.fold("")(v => s": $v")}"
    }
  }
