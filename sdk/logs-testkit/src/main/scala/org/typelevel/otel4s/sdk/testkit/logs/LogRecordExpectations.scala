...
object LogRecordExpectations {
  ...
  private[logs] def renderRecord(record: LogRecordData): String = {
    val severity = record.severity.fold("UNSPECIFIED")(_.toString)
    val eventName = record.eventName.fold("")(name => s"$name ")
    val body = record.body.fold("<missing>")(v => s"'$v'")
    s"[$severity ${eventName}${body}]"
  }
  ...
  private def bestMismatch(records: List[LogRecordData], expectation: LogRecordExpectation): LogRecordMismatch = {
    records
      ...
      .sortBy { case (_, mismatches) =>
        val bodyMismatch = mismatches.exists {
          case _: LogRecordExpectation.Mismatch.BodyMismatch => true
          case _                                             => false
        }
        val severityMismatch = mismatches.exists {
          case _: LogRecordExpectation.Mismatch.SeverityMismatch     => true
          case _: LogRecordExpectation.Mismatch.SeverityTextMismatch => true
          case _                                                     => false
        }
        val eventNameMismatch = mismatches.exists {
          case _: LogRecordExpectation.Mismatch.EventNameMismatch => true
          case _                                                  => false
        }
        ...
        (bodyMismatch, severityMismatch, eventNameMismatch, traceCorrelationMismatch, mismatches.length)
      }
      .headOption
