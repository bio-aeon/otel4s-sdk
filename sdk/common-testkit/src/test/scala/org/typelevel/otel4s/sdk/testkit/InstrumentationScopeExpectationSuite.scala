...
class InstrumentationScopeExpectationSuite extends FunSuite {
  ...
  test("attributesExact reports nested attribute failures") {
    ...
    assertEquals(expectation.check(scope(attributes = Attributes(Attribute("scope.attr", "value")))), Right(()))
    assertEquals(
      expectation.check(scope(attributes = Attributes(Attribute("scope.attr", "other")))),
      Left(
        NonEmptyList.one(
  ...
  test("attributesSubset matches contained attributes") {
    ...
    assertEquals(
      expectation.check(scope(attributes = Attributes(Attribute("scope.attr", "value"), Attribute("other", "x")))),
      Right(())
    )
    assertEquals(
      expectation.check(scope(attributes = Attributes(Attribute("other", "x")))),
      Left(
        NonEmptyList.one(
          InstrumentationScopeExpectation.Mismatch.attributesMismatch(
  ...
  test("exact matches the full scope") {
    val actual = scope(
      ...
      attributes = Attributes(Attribute("scope.attr", "value"))
    )
