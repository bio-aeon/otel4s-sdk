...
instrumentation scope, telemetry resource, point attributes, collection windows, exemplars, histogram buckets,
and more.
...
Use `points(PointSetExpectation.exists(...))` directly when you need subset matching or a more detailed point
expectation.
...
As with points, exemplar expectations are partial. You can assert only the value, only filtered attributes, only
trace context, or any combination of them.
...
For numeric metrics, the `points` passed to `pointsWhere` are typed numeric points, so you can inspect `.value`,
`.attributes`, and exemplar data directly.
...
After that, double-based metric, point, and exemplar expectations built in that scope will use the custom
comparison.
